...
export const getWikipediaSummary = internalAction({
    ...
    handler: async (ctx, args) => {
        const response = await fetch("https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles="  + encodeURIComponent(args.topic));
        const summary = getSummaryFromJSON(await response.json());
        await ctx.scheduler.runAfter(0, api.chat.sendMessage, {
            user: "Wikipedia Bot",
            body: summary || `No summary found for topic "${args.topic}".`
        });
    }
});
